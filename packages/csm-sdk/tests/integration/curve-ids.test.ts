...
// Pins the static per-chain curve-id table (OPERATOR_TYPE_CURVE_ID) to
// on-chain truth: gate contracts own their curve id via curveId()/CURVE_ID().
// If a curve assignment changes or a new gate lands on the forked chain, this
// fails instead of letting the labeling table drift silently.
...
describe('integration: OPERATOR_TYPE_CURVE_ID matches on-chain gate curves', () => {
  it('CSM gates report the curve ids from the static table', async () => {
    const sdk = useCsmSdk();
    const table = OPERATOR_TYPE_CURVE_ID[sdk.core.chainId];
  ...
  it('CM curated gates report the curve ids from the static table', async () => {
    ...
    for (const [gateName, gate] of sdk.curatedGates.getAll()) {
      ...
      expect(curveId, `gate ${gateName}`).toBe(
        table[CURATED_GATE_TYPES[gateName]],
      );
    }
  });
});
